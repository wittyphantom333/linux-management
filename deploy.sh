...
if [ -f "$ENV_FILE" ]; then
    ...
    if ! grep -q '^COMPOSE_FILE=' "$ENV_FILE" 2>/dev/null; then
        ...
        echo "" >> "$ENV_FILE"
        echo "# --- Docker Compose ---" >> "$ENV_FILE"
        echo "# Ensures 'docker compose up -d' automatically uses the build overlay." >> "$ENV_FILE"
        echo "COMPOSE_FILE=docker-compose.yml:docker-compose.build.yml" >> "$ENV_FILE"
        success "COMPOSE_FILE added to .env"
    fi

    success "Existing .env preserved. Edit $ENV_FILE to make changes."
else
    info "Creating new .env at: $ENV_FILE"
    ...
    # Write .env line by line (avoids heredoc variable expansion issues)
    {
        echo "# ============================================================================="
        echo "# PatchMon Environment Configuration"
        echo "# Generated by deploy.sh on $(date '+%Y-%m-%d %H:%M:%S')"
        echo "# ============================================================================="
        echo ""
        echo "# --- Docker Compose ---"
        echo "# This tells 'docker compose' to always load both files so you can just run:"
        echo "#   docker compose up -d"
        echo "#   docker compose logs -f"
        echo "#   docker compose restart"
        echo "# without needing -f flags every time."
        echo "COMPOSE_FILE=docker-compose.yml:docker-compose.build.yml"
        echo ""
        echo "# --- Required ---"
    ...
    if [ "$ENV_LINES" -lt 5 ] || [ "$ENV_SIZE" -lt 50 ]; then
        error ".env file appears incomplete ($ENV_LINES lines, $ENV_SIZE bytes). Contents:"
        cat "$ENV_FILE" >&2
        die "Failed to write .env — check disk space and permissions."
    fi

    success "Environment file created at $ENV_FILE ($ENV_LINES lines, $ENV_SIZE bytes)"
    info "Verifying secrets are populated..."

    # Extract and verify the written values
    ...
    success "All secrets verified in .env (PG:${#WRITTEN_PG}c, Redis:${#WRITTEN_RD}c, JWT:${#WRITTEN_JWT}c)"
fi
